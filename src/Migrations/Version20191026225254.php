 ...
 */
final class Version20191026225254 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__article AS SELECT id, user_id, title, text, date, thumbnail FROM article');
        $this->addSql('DROP TABLE article');
        $this->addSql('CREATE TABLE article (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, title VARCHAR(100) NOT NULL COLLATE BINARY, text VARCHAR(280) NOT NULL COLLATE BINARY, date DATETIME NOT NULL, thumbnail VARCHAR(255) DEFAULT NULL, CONSTRAINT FK_23A0E66A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO article (id, user_id, title, text, date, thumbnail) SELECT id, user_id, title, text, date, thumbnail FROM __temp__article');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__article AS SELECT id, user_id, title, text, date, thumbnail FROM article');
        $this->addSql('DROP TABLE article');
        $this->addSql('CREATE TABLE article (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, title VARCHAR(100) NOT NULL, text VARCHAR(280) NOT NULL, date DATETIME NOT NULL, thumbnail VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO article (id, user_id, title, text, date, thumbnail) SELECT id, user_id, title, text, date, thumbnail FROM __temp__article');
        $this->addSql('DROP TABLE __temp__article');
        $this->addSql('CREATE INDEX IDX_23A0E66A76ED395 ON article (user_id)');
        $this->addSql('DROP INDEX IDX_10FA01987294869C');
